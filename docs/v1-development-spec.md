...
本说明书保留完整平台的技术基线，但当前实现与验收必须先遵守[本命盘优先开发计划](./natal-development-plan.md)、[对象库/新建分析/后台管理计划](./admin-object-analysis-plan.md)和[完整本命盘纵向切片契约](./natal-first-slice.yaml)。除全局入口预留外，行运、返照、关系和其他新技法全部暂停；`NATAL-RELEASE`通过后也不得自动恢复，必须等待用户明确授权。当前开发任务、接口、数据模型、测试和发布验收共同以这些当前计划、本文、[专业工作台、分析选择与参数契约](./professional-workspace-contract.yaml)、[Canonical JSON Schema](../packages/canonical-schema/README.md)、[OpenAPI 3.1](../openapi/openapi.yaml)、[算法卡目录](../algorithm-cards/catalog.yaml)和[测试规范](../tests/)为准。涉及占星流派差异或复杂公式的工作项，必须先根据[算法卡模板](./algorithm-card-template.md)完成算法卡；没有算法卡的能力不得标记为 `Stable`。其余完整计算和图形目录继续保留，但不授权当前实现。
...
```text
顶部：工作台 / 分析中心 / 技法排盘 / 对象库 / 新建分析 / 账户
左侧：当前对象 / 最近对象 / 最近分析 / 进入对象库
中央：个人仪表盘、对象库、对象页、分析构建器、结果工作台、图表视图或报告阅读器
结果导航：概览 / 星盘与图表 / 时间线 / 综合分析 / 数据与证据
...
“技法排盘”面向专业用户，以用途、输入、输出、流派、参数和限制解释盘型能力；原始ID默认折叠。顶部不再出现“计算方法”和“报告”：专题/目的在分析中心选择，综合报告与AI文本在对应结果和对象档案中就近查看。“能力注册表、数据集与许可证、Schema、API和系统状态”属于开发者/运维信息，移到`账户→设置→开发者`。结果页“数据与证据”只展示本次使用的引擎、规则、参数、数据版本、可信度和证据。
...
每个个人Workspace最多有一个活动的“主要本人档案”，也允许没有。其他人物以本人之外的独立Subject保存，并可标记伴侣、父母、子女、兄弟姐妹、亲属、朋友、同事、客户或其他关系。对象库使用独立`/objects`路由，承载人物、关系、事件、项目、组织、地点和问题/案例；用户中心只提供账户资料、安全、隐私、默认偏好、本人档案摘要、最近对象/分析和进入对象库的快捷入口。登录用户在分析中临时录入人物时，“保存到对象库”默认关闭；结果页允许转存，但只能创建新Subject/SubjectVersion和非破坏性关联，不得重写既有Snapshot。
...
→ 2 选择已有对象或录入临时对象和上下文
→ 3 选择推荐方案并确认5—7个关键参数
→ 4 预览实际生效参数、扩展、图表、综合分析和降级
→ 5 确认保存语义并运行
```

第一步的每张技法、专题和目的卡必须以用户语言说明“分析什么、需要什么输入、主要输出什么、不包含什么、成熟度如何”；`natal.standard_chart`等内部ID只能出现在折叠技术详情或专家模式，不能占据主要副标题。

第三步将AnalysisModel显示为“推荐方案/分析体系/流派预设”。技法入口可在兼容范围内切换现代、古典、希腊化和综合体系；专题和目的入口展示解析结果，只开放模型版本声明的覆盖项。每个方案必须显示适用场景、`preset_id`/版本、公开来源或平台验证依据、与其他方案的差异和允许覆盖项，并使用“平台推荐”“常用配置”或具体流派标签；不得宣称没有证据的行业唯一权威。快捷设置包含体系、黄道、宫位制、相位集、容许度、点集和轮盘模式；高级设置按黄道/Ayanamsa、中心与坐标、宫位、天体与虚点、小行星/固定星/TNP、相位与容许度、阿拉伯点、古典表、时间技法和关系方向性分组。完整字段、集合和覆盖优先级必须实现`professional-workspace-contract.yaml#professional_parameters`，不能仅依赖无约束`custom_parameters`。
...
信息架构验收以“用户下一步是否唯一且可解释”为准：默认台面只展示示例或当前对象、已有缓存、唯一`新建分析`主动作和最近计算；三种选择目录在分析中心内展示，三种上下文入口只作为对象页和快捷动作预填。计算目录、图表目录和综合分析内容属于不同任务空间，不在同一屏平铺全部能力。顶部不设置独立报告入口；已有报告在对象或结果上下文中可达。高频操作就近展示，低频专业参数进入检查器或二级面板；用户始终能看到当前对象、当前入口、当前选择、是否会计算以及返回修改路径。

### 8.10 独立对象库与后台管理

对象库使用`/objects`，用户中心不重复实现人物/关系/事件列表。首发完整支持人物对象和最新本命结果恢复；关系、事件、项目、组织、地点和问题/案例只有在Schema、API、账户隔离、删除和真实计算消费入口同时完成后才启用创建。未实现类型必须禁用并说明阶段，不得套用人物表单或返回本命占位结果。

后台使用`/admin`和`/api/v1/admin/*`，不出现在普通用户导航。首发只建立`user`、`admin`、`super_admin`三种角色以及`active/suspended/disabled/deletion_pending/deleted`账户状态；服务端必须执行角色和状态校验，停用/禁用同时撤销全部会话，最后一个超级管理员不得删除或降级。

后台首批能力包括：用户搜索和状态、注册/最近登录、对象/计算/AI用量、受审计的新增/停用/禁用/恢复/删除、管理员管理、平台AI/API提供方密钥管理、平台AI预分析提示词、HTTP/业务/AI用量聚合和管理审计。行为事件禁止包含出生资料、姓名、私人对象名、星盘结果、报告或AI正文、密码、令牌、完整密钥和提示词正文；不做会话回放、键盘或鼠标监控。

当前实现没有套餐、逐功能授权和额度模型，因此后台不显示“用户权限”矩阵。用量不是权限：本轮只实施角色和数据所有者隔离；月度计算、AI令牌、对象数、存储、API速率、导出和Beta功能等`Entitlement/Quota`等待计费政策确定后另行开发。平台API Key保存后只显示掩码/末4位，完整值只能位于服务端加密存储、容器Secret或服务器环境变量，不得进入前端、Git、日志和行为事件。

平台AI预分析提示词支持一个全局默认版本和`provider_id + model_id`覆盖版本。每次用户AI调用按“代码内不可变安全边界 → 生效平台提示词 → 用户分析重点/输出格式 → 确定性分析文档”装配；管理员不能覆盖安全边界或授权模型改写计算事实。AI产物必须记录生效提示词版本、内容哈希、提供方和精确模型；提示词正文只在管理员受限接口内可见，不进入普通用户接口、客户端、日志、行为事件和导出。发布、回滚、覆盖和测试均须审计。完整状态机、字段、接口和E2E以[`admin-object-analysis-plan.md`](./admin-object-analysis-plan.md)为准。
