...
| 范围 | iOS 十二盘（六个核心卡片盘 + 六个高级盘）、v6 Today、AI Artifact、Go Relay、`/xiaoguiwk` |
...
- 六个核心卡片盘：本命 10、天象 7、行运 6、次限 6、日返 7、合盘 8；另有六个高级盘按需计算并生成整盘报告；
...
- ChatGPT 只辅助优化用户主动请求的整盘报告，不计算星盘，也不生成 App 内固定卡片或解释内容。Relay 转发给 ChatGPT 的内容仅限名字/昵称、适用时的关系类型和设备已计算结果；不得转发 Profile 出生日期、出生时间、出生地点、地点名称、经纬度、时区或原始星盘时间元数据。用户可用昵称代替真实姓名；计算事件日期和时间窗口作为结果事实可随证据发送；
...
- 同一 Apple 购买身份每个 UTC 自然月最多获得一次 2 个免费 Credits；Pro 按订阅锚点的每个额度周期最多获得一次 10 个 Pro Credits，两类月度额度均为替换而非累加；Annual 首次购买一次性发放 20 Bonus Credits、1 年过期。删除/重建 User ID、恢复、续订与交易重放均不得重复发放当期额度或首次奖励；
- Credit 消耗顺序固定为 allowance → 有期限的 bonus/admin → purchased；购买的 Credits 永不过期。Relay 是余额、预留、消费、释放与 Ledger 的唯一权威；
- 匿名 `userID` 由首次启动生成并保存到 Keychain，与 installation ID 分离；Profile 底部以小字展示可复制 User ID，StoreKit 使用 `appAccountToken=userID`。删除账户时清除本机/iCloud 个人资料、人物、报告、Ask、设置和全部可用 Credits，Relay 原子停用旧 ID 并创建空的新 ID；旧 ID、根/前后继关系以及购买、订阅和 Credit 财务历史只在 Relay 保留。Apple 订阅不会随删除取消或自动迁移，用户恢复购买时必须验证 Apple 签名交易、校验同一 ID 链，并通过 App Store Server API 把订阅 `appAccountToken` 重绑到当前新 ID；
...
- Relay Users 同时展示 Active/Inactive、首个/前任/后继 User ID；停用卡只读保留购买、订阅、Credit Grant/Ledger 财务历史，不保留报告请求、交付 payload 或其他个人内容，也不允许后台调整 Plan/Credits；
