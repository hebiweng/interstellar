...
- [x] Pro Credits 政策已统一为新用户首月 Free 5、后续每月 Free 2、Pro 每月额外 10；Annual Pro 首购另送 20，首月年付 Pro 新用户总额为 35。iOS Guide、付费墙、法律条款、StoreKit 描述与 Relay 常量及回归测试均已同步；Build 18（`1.0 (18)`）Archive 已上传 App Store Connect，返回 `Upload succeeded` / `EXPORT SUCCEEDED`，等待处理。生产 Relay `interstellar-relay:v6-20260902-build25-pro-credits` 已部署；切换前 SQLite backup 为 `/opt/interstellar/backups/relay-before-build25-pro-credits-20260902T121938Z.db`，备份及切换后 integrity 均为 `ok`，容器 healthy、RestartCount=0，公开健康检查正常，Edge/Caddy 未重建。
