...
} from "./components/lib/chart-types";
import {
  globalNavigation,
  entryModes,
  chartTechniques,
  ...
  sampleSnapshot,
  defaultPerson,
} from "./components/lib/chart-constants";
import { aspectPhaseLabel } from "./components/lib/chart-labels";
import {
  effectivePointIds,
  formatDegree,
  pointPlacementLabel,
  ...
  isDateLevelSnapshot,
} from "./components/lib/chart-utils";
import {
  buildLocalTechnicalDocument,
  SafeMarkdownDocument,
} from "./components/lib/local-interpretation";

import { NatalWheel } from "./components/wheels/natal-wheel";
import { AspectGrid } from "./components/wheels/aspect-grid";
import { TimeDependentUnavailable } from "./components/results/time-dependent-unavailable";
import { PointResultTable } from "./components/results/point-result-table";
import { CalculationResults } from "./components/results/calculation-results";
import { StructureResults } from "./components/results/structure-results";
import { NatalFeatureResults } from "./components/results/natal-feature-results";
import { ClassicalResults } from "./components/results/classical-results";
import { InterpretationDrawer } from "./components/panels/interpretation-drawer";
import { PersonFields } from "./components/forms/person-fields";

const CurrentSkyWorkspace = dynamic(() => import("./components/workspaces/current-sky-workspace").then((m) => m.CurrentSkyWorkspace), {
  loading: () => <section className="main-workspace empty-workspace"><div><span>◌</span><h1>正在加载当前天象工作台</h1><p>正在准备组件资源。</p></div></section>,
  ssr: false,
});
const TransitWorkspace = dynamic(() => import("./components/workspaces/transit-workspace").then((m) => m.TransitWorkspace), {
  loading: () => <section className="main-workspace empty-workspace"><div><span>◌</span><h1>正在加载推运工作台</h1><p>正在准备组件资源。</p></div></section>,
  ssr: false,
});
const SecondaryProgressionsWorkspace = dynamic(() => import("./components/workspaces/secondary-progressions-workspace").then((m) => m.SecondaryProgressionsWorkspace), {
