...
import { createDodecatemoria } from '../../lib/interstellar-api';
...
export function DodecatemoriaWorkspace({
  theme,
  person,
  latestNatalSnapshot,
  savedPeople,
  ...
  const dodecAutoCalculated = useRef(false);
  ...
  const calculate = useCallback(async () => {
    ...
    setBusy(true);
    try {
      const calculated = await createDodecatemoria(
        latestNatalSnapshot,
        person,
        requestSettings,
      );
  ...
  useEffect(() => {
    if (dodecAutoCalculated.current) return;
    dodecAutoCalculated.current = true;
    ...
    <div className="workbench-grid">
      <article className="wheel-panel chart-workspace-card">
        <div className="panel-heading">
          <div><small>DODECATEMORIA</small><h2>12分轮盘</h2></div>
          <div className="panel-tools">
              ...
              <button className="natal-guide-link" onClick={() => setGuideOpen(true)}>什么是12分盘？</button>
            </div>
        </div>
        <div className="wheel-canvas-area">
          {harmonicSnapshot && comparison ? (
            chartView === "aspect_grid" ? (
              ...
              <ComparisonWheel natalSnapshot={latestNatalSnapshot} movingSnapshot={harmonicSnapshot} comparison={comparison} renderSpec={natalRenderSpec} controls={controls} chartLabel="12分盘" movingLabel="12分外层" hideLegend />
            ) : harmonicRenderSpec ? (
              <NatalWheel snapshot={harmonicSnapshot} renderSpec={harmonicRenderSpec} controls={controls} />
            ) : null
          ) : <div className="sky-empty-state"><span>⑫</span><h3>12分盘尚未计算</h3><p>12分盘将每颗行星的黄经乘以12后取模360°，揭示隐藏在行星度数中的深层信息。点击计算即可生成。</p><button onClick={() => void calculate()} disabled={busy}>{busy ? "计算中…" : "计算12分盘"}</button></div>}
        </div>
        <footer><span>{person.displayName} · {person.localDate}</span><span>黄经×12</span><span>{appliedSettings.zodiac === "tropical" ? "回归黄道" : "恒星黄道"}</span></footer>
      </article>

      <aside className="settings-panel settings-panel-secondary">
        <div className="settings-title"><div><small>DODECATEMORIA SETTINGS</small><h2>12分盘参数</h2></div><div className="settings-title-actions"><button className="advanced-settings-trigger" onClick={() => setDrawerOpen(true)}>专业参数</button></div></div>
        ...
        <div className="secondary-fields">
          <small className="secondary-field-hint">12分盘无需额外参数，直接基于本命星体度数计算</small>
        </div>

        <div className="preset-shortcuts" aria-label="12分盘预设">{activePresets.map((preset) => <button key={preset.id} className={presetId === preset.id ? "active" : ""} onClick={() => applyPreset(preset.id)}><b>{preset.label}</b><small>{preset.badge}</small></button>)}</div>

        <button className="settings-calculate" disabled={busy} onClick={() => void calculate()}>{busy ? "计算中…" : "计算12分盘"}</button>

        <p className="settings-boundary">12分盘无需额外参数，直接基于本命星体度数计算。专业参数（黄道制、宫位制、交点类型等）点击上方「专业参数」按钮配置。</p>
      </aside>
      ...
      {drawerOpen && <div className="drawer-backdrop" onClick={() => setDrawerOpen(false)}><aside className="settings-drawer" onClick={(e) => e.stopPropagation()}>
        <header className="drawer-header"><h2>12分盘专业参数</h2><button className="drawer-close" onClick={() => setDrawerOpen(false)} aria-label="关闭">×</button></header>
        <div className="drawer-body">
          <p className="sky-person-boundary"><b>固定本命：{person.displayName}</b><span>12分盘基于本命星体度数乘以12后取模计算，无需额外时间或地点参数。</span></p>
          ...
          <SharedAdvancedCalculationFields settings={settings} groups={groups} chartLabel="12分盘" onSettingsChange={setSettings} onGroupsChange={setGroups} />
        </div>
      ...
      <aside className="ai-insight-panel">
        <header><div><small>DODECATEMORIA INSIGHT · LOCAL</small><h2>度数中的秘密</h2></div></header>
        <div className="ai-waiting"><b>解读开发中</b><p>12分盘的本地即时解读和 AI 分析功能正在开发中。计算接口就绪后将首先提供本地解读。</p></div>
        <footer><span>本地即时解读</span><small>开发中</small></footer>
      </aside>
    </div>

    <TechniqueGuideDialog open={guideOpen} title="什么是12分盘？" path="/dodecatemoria-guide.md" onClose={() => setGuideOpen(false)} />
  </section>;
}
