...
import { buildFirdariaRightPanel, type FirdariaRightPanel } from "../../lib/insight/firdaria";

type FirdariaInstantInsightProps = {
...
export function FirdariaInstantInsight({ latestNatalSnapshot }: FirdariaInstantInsightProps) {
  const rightPanel: FirdariaRightPanel = useMemo(
    () => buildFirdariaRightPanel(latestNatalSnapshot),
    [latestNatalSnapshot],
  );
  ...
  return (
    <div className="instant-insight">
      ...
      {/* Card C: 昼夜起序 */}
      {cardC && <section className="insight-card-sp insight-card-timeline">
        <header className="insight-card-sp-header">
