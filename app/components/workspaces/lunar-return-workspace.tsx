...
import type { NatalCalculationSettings, NatalSnapshot, NatalPersonInput, LunarReturnResult } from '../../lib/interstellar-api';
import type { ThemeMode } from '../lib/chart-types';
import type { WorkspacePerson } from '../../lib/account-workspace';
import { createLunarReturn } from '../../lib/interstellar-api';
...
export function LunarReturnWorkspace({
  theme,
  person,
  latestNatalSnapshot,
  savedPeople,
  ...
  const [result, setResult] = useState<LunarReturnResult | null>(null);
  ...
  const returnRenderSpec = useMemo(
    () => returnSnapshot ? buildNatalRenderSpec(returnSnapshot, chartView === "compact" ? "compact" : "professional", theme, controls, "current_sky") : null,
  ...
  const calculate = useCallback(async () => {
    ...
    setBusy(true);
    try {
      const calculated = await createLunarReturn(
        latestNatalSnapshot,
        person,
        Number(latitude),
        Number(longitude),
    ...
    <div className="workbench-grid">
      <article className="wheel-panel chart-workspace-card">
        <div className="panel-heading">
          <div><small>LUNAR RETURN</small><h2>月返轮盘</h2></div>
          <div className="panel-tools">
              ...
              <button className="natal-guide-link" onClick={() => setGuideOpen(true)}>什么是月返盘？</button>
            </div>
        </div>
        <div className="wheel-canvas-area">
          {returnSnapshot && comparison ? (
            chartView === "aspect_grid" ? (
              ...
              <ComparisonWheel natalSnapshot={innerNatal!} movingSnapshot={outerMoving!} comparison={comparison} renderSpec={natalRenderSpec} controls={controls} chartLabel="月返盘" movingLabel={innerChart === "natal" ? "月返外层" : "本命外层"} hideLegend />
            ) : returnRenderSpec ? (
              <NatalWheel snapshot={returnSnapshot} renderSpec={returnRenderSpec} controls={controls} />
            ) : null
          ) : <div className="sky-empty-state"><span>☽</span><h3>月返盘尚未计算</h3><p>月返盘是月亮回归到出生黄经度数时的天象盘，约每27.3天一次，用于解读该月度的情感主题与生活焦点。配置地点参数后点击计算。</p><button onClick={() => void calculate()} disabled={busy}>{busy ? "计算中…" : "计算月返盘"}</button></div>}
        </div>
        <footer><span>{person.displayName} · {person.localDate}</span><span>月亮回归</span><span>{result ? `月返时刻 ${result.return_time_utc.replace("T", " ")}` : "约每27.3天一次"}</span><span>{appliedSettings.zodiac === "tropical" ? "回归黄道" : "恒星黄道"}</span></footer>
      </article>

      <aside className="settings-panel settings-panel-secondary">
        <div className="settings-title"><div><small>LUNAR RETURN SETTINGS</small><h2>月返盘参数</h2></div><div className="settings-title-actions"><button className="advanced-settings-trigger" onClick={() => setDrawerOpen(true)}>专业参数</button></div></div>
        ...
        <div className="secondary-fields">
          ...
          <small className="secondary-field-hint">月返盘以居住地经纬度计算宫位</small>
        </div>

        <div className="preset-shortcuts" aria-label="月返盘预设">{activePresets.map((preset) => <button key={preset.id} className={presetId === preset.id ? "active" : ""} onClick={() => applyPreset(preset.id)}><b>{preset.label}</b><small>{preset.badge}</small></button>)}</div>

        <button className="settings-calculate" disabled={busy} onClick={() => void calculate()}>{busy ? "计算中…" : "计算月返盘"}</button>
      ...
      {drawerOpen && <div className="drawer-backdrop" onClick={() => setDrawerOpen(false)}><aside className="settings-drawer" onClick={(e) => e.stopPropagation()}>
        <header className="drawer-header"><h2>月返盘专业参数</h2><button className="drawer-close" onClick={() => setDrawerOpen(false)} aria-label="关闭">×</button></header>
        <div className="drawer-body">
          <p className="sky-person-boundary"><b>固定本命：{person.displayName}</b><span>本命点位与宫位直接读取上一次结果；默认只给月返层加载现代预设。</span></p>
          ...
          <label className="check-option"><input type="checkbox" checked={precessionCorrection} onChange={(e) => setPrecessionCorrection(e.target.checked)} /><span>消除月返岁差</span><small>开启后月返计算将消除岁差影响</small></label>
          ...
          <SharedAdvancedCalculationFields settings={settings} groups={groups} chartLabel="月返盘" onSettingsChange={setSettings} onGroupsChange={setGroups} />
        </div>
      ...
      <aside className="ai-insight-panel">
        <header><div><small>LUNAR RETURN INSIGHT · LOCAL</small><h2>这一月的主题</h2></div></header>
        <div className="ai-waiting"><b>解读开发中</b><p>月返盘的本地即时解读和 AI 分析功能正在开发中。计算接口就绪后将首先提供本地解读。</p></div>
        <footer><span>本地即时解读</span><small>开发中</small></footer>
      </aside>
    </div>

    <TechniqueGuideDialog open={guideOpen} title="什么是月返盘？" path="/lunar-return-guide.md" onClose={() => setGuideOpen(false)} />
  </section>;
}
