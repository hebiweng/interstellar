...
final class AppChartCalculationService {
    ...
    /// Calculates one chart for an immutable Theme task without mutating the
    /// Charts screen state. Themes orchestrate existing chart techniques; the
    /// deterministic astrology remains in the shared calculation layer.
    func calculateThemeChart(
        chart: ChartKind,
        profile: UserProfile,
        targetDate: Date,
        ...
        switch chart {
        case .natal:
            let snapshot = try await calculator.calculateSnapshot(natalInput, preset: preset)
            return ChartDisplayResult(
                snapshot: snapshot,
                reference: nil,
                comparisonAspects: snapshot.aspects,
                techniqueMetadata: .standard
            )
        ...
        case .transit:
            ...
            return ChartDisplayResult(
                snapshot: snapshot,
                reference: reference,
                comparisonAspects: SwissEphemerisCalculator.compare(
                    moving: snapshot,
                    reference: reference,
        ...
        case .solarReturn:
            ...
            return ChartDisplayResult(
                snapshot: snapshot,
                reference: reference,
                comparisonAspects: SwissEphemerisCalculator.solarReturnNatalAspects(
                    solarReturn: snapshot,
                    natal: reference
                ),
                techniqueMetadata: .standard
            )
