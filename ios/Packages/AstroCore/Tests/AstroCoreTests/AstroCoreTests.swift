...
struct AstroCoreTests {
    ...
    @Test("Traditional dignity tables resolve sect triplicity Egyptian terms and Chaldean faces")
    func traditionalDignityTables() {
        #expect(HoraryEngine.triplicityRuler(ofSign: 0, isDayChart: true) == .sun)
        #expect(HoraryEngine.triplicityRuler(ofSign: 0, isDayChart: false) == .jupiter)
        #expect(HoraryEngine.termRuler(ofSign: 0, degreeInSign: 0) == .jupiter)
        #expect(HoraryEngine.termRuler(ofSign: 0, degreeInSign: 6) == .venus)
        #expect(HoraryEngine.termRuler(ofSign: 6, degreeInSign: 13.999) == .mercury)
        #expect(HoraryEngine.termRuler(ofSign: 6, degreeInSign: 14) == .jupiter)
        #expect(HoraryEngine.faceRuler(ofSign: 0, degreeInSign: 0) == .mars)
        #expect(HoraryEngine.faceRuler(ofSign: 0, degreeInSign: 10) == .sun)
        #expect(HoraryEngine.faceRuler(ofSign: 1, degreeInSign: 0) == .mercury)
    }

    @Test("Reception records all five essential dignity channels")
    func fullReceptionChannels() {
        let reception = HoraryEngine.reception(
            from: .sun,
            to: .mars,
            fromSignIndex: 0,
            degreeInSign: 2,
        ...
        #expect(reception.byDomicile)
        #expect(!reception.byExaltation)
        #expect(reception.dignities.contains(.domicile))
        #expect(reception.dignities.contains(.face))
        #expect(reception.strength > 0)
    }

    @Test("Moon testimony keeps previous next and upcoming aspects before sign exit")
    func moonTestimonySequence() async throws {
        let calculator = try SwissEphemerisCalculator(ephemerisDirectory: ephemerisDirectory)
        let snapshot = try await calculator.calculateSnapshot(
            NatalInput(
        ...
        #expect((moon.hoursUntilSignExit ?? 0) > 0)
        #expect(moon.upcomingAspects == moon.upcomingAspects.sorted { $0.hoursFromQuestion < $1.hoursFromQuestion })
        #expect(moon.upcomingAspects.allSatisfy { $0.aspect.phase == .applying })
        if let previous = moon.previousAspect {
            #expect(previous.aspect.phase == .separating)
            #expect(previous.hoursFromQuestion < 0)
        }
        if let next = moon.nextAspect {
            #expect(moon.upcomingAspects.first?.aspect.id == next.id)
        }
    }

    @Test("Moon testimony resolves exact ephemeris events within the current sign")
    func exactMoonTestimony() async throws {
        let calculator = try SwissEphemerisCalculator(ephemerisDirectory: ephemerisDirectory)
        let snapshot = try await calculator.calculateSnapshot(
            NatalInput(
                utcDate: Date(timeIntervalSince1970: 1_775_000_000),
        ...
        let testimony = try await calculator.resolveHoraryMoonTestimony(snapshot: snapshot)
        #expect((testimony.hoursUntilSignExit ?? 0) > 0)
        #expect(testimony.upcomingAspects == testimony.upcomingAspects.sorted { $0.hoursFromQuestion < $1.hoursFromQuestion })
        #expect(testimony.upcomingAspects.allSatisfy { $0.hoursFromQuestion > 0 })
        if let previous = testimony.previousAspect {
            #expect(previous.hoursFromQuestion <= 0)
            #expect(previous.aspect.phase == .separating)
        }
        #expect(testimony.nextAspect?.id == testimony.upcomingAspects.first?.aspect.id)
    }

    @Test("Horary considerations are cautions rather than a hard validity gate")
    func horaryConsiderations() async throws {
        let calculator = try SwissEphemerisCalculator(ephemerisDirectory: ephemerisDirectory)
        let location = GeographicLocation(latitudeDegrees: 35.0263, longitudeDegrees: 111.0073)
        let anchor = Date(timeIntervalSince1970: 1_775_000_000)
        var earlySnapshot: ChartSnapshot?
        for hour in 0 ..< 48 {
            let snapshot = try await calculator.calculateSnapshot(
                NatalInput(utcDate: anchor.addingTimeInterval(Double(hour) * 3_600), location: location),
                configuration: .horary
            )
            let ascDegree = snapshot.angles.ascendantDegrees.truncatingRemainder(dividingBy: 30)
            if ascDegree < 3 {
                earlySnapshot = snapshot
                break
            }
        }
        let snapshot = try #require(earlySnapshot)
        let assessment = HoraryEngine.considerations(in: snapshot)
        #expect(assessment.flags.contains { $0.kind == .earlyAscendant })
        #expect(assessment.reliability != .high)
        #expect(!assessment.flags.isEmpty)
        #expect(HoraryEngine.isViaCombusta(longitudeDegrees: 200))
        #expect(!HoraryEngine.isViaCombusta(longitudeDegrees: 190))
    }
    ...
    @Test("Lilly timing maps sign and house qualities without inventing a future search")
    func lillyTimingQualities() {
        #expect(HoraryTimingEngine.signQuality(signIndex: 0) == .moveable)
        #expect(HoraryTimingEngine.signQuality(signIndex: 3) == .moveable)
        #expect(HoraryTimingEngine.signQuality(signIndex: 1) == .fixed)
        #expect(HoraryTimingEngine.signQuality(signIndex: 2) == .common)
        #expect(HoraryTimingEngine.houseQuality(house: 1) == .angular)
        #expect(HoraryTimingEngine.houseQuality(house: 11) == .succedent)
        #expect(HoraryTimingEngine.houseQuality(house: 6) == .cadent)
    }

    @Test("Lilly timing uses degrees to a completing perfection as symbolic units")
    func lillyTimingFromPerfection() async throws {
        let calculator = try SwissEphemerisCalculator(ephemerisDirectory: ephemerisDirectory)
        let snapshot = try await calculator.calculateSnapshot(
            NatalInput(
                utcDate: Date(timeIntervalSince1970: 1_775_000_000),
        ...
        let applying = try #require(snapshot.points.first?.body)
        let receiving = try #require(snapshot.points.dropFirst().first?.body)
        let path = HoraryPerfectionPath(
            kind: .direct,
            exactDate: snapshot.utcDate.addingTimeInterval(3 * 86_400),
            aspectKind: .trine,
            distanceDegrees: 3.25,
            mediator: nil,
            applyingBody: applying,
            receivingBody: receiving
        )
        let result = HoraryTimingEngine.interpret(
        ...
        #expect(result.status == .indicated)
        #expect(abs((result.symbolicUnits ?? 0) - 3.25) < 0.000_001)
        #expect(result.testimonies.contains { $0.role == .applyingSign })
        #expect(result.testimonies.contains { $0.role == .applyingHouse })
        #expect(result.testimonies.contains { $0.role == .receivingSign })
        #expect(result.testimonies.contains { $0.role == .receivingHouse })
        #expect(result.testimonies.contains { $0.role == .moonSign })
        #expect(result.testimonies.contains { $0.role == .moonHouse })
        #expect(!result.scales.isEmpty)
        #expect(result.exactPerfectionDate == path.exactDate)
        #expect(result.evidenceIDs.contains("lilly.timing.degrees-to-perfection"))
    }

    @Test("Lilly timing refuses a timing promise when perfection is prevented")
    func lillyTimingRequiresPerfection() async throws {
        let calculator = try SwissEphemerisCalculator(ephemerisDirectory: ephemerisDirectory)
        let snapshot = try await calculator.calculateSnapshot(
            NatalInput(
                utcDate: Date(timeIntervalSince1970: 1_775_000_000),
        ...
        #expect(result.status == .prevented)
        #expect(result.symbolicUnits == nil)
        #expect(result.exactPerfectionDate == nil)
    }
    ...
    func electionTimingSearch() async throws {
        ...
        #expect(candidates == candidates.sorted { $0.assessment.suitabilityScore > $1.assessment.suitabilityScore })
        #expect(candidates.allSatisfy { (0 ... 100).contains($0.assessment.suitabilityScore) })
        #expect(candidates.allSatisfy { !$0.assessment.factors.isEmpty })
        #expect(candidates.allSatisfy { $0.legacyHoraryAnalysis == nil })
    ...
    @Test("Electional candidate decodes legacy Horary analysis without re-encoding it")
    func electionalCandidateLegacyDecode() async throws {
        let calculator = try SwissEphemerisCalculator(ephemerisDirectory: ephemerisDirectory)
        let snapshot = try await calculator.calculateSnapshot(
            NatalInput(
                utcDate: Date(timeIntervalSince1970: 1_775_000_000),
        ...
        let analysisObject = try JSONSerialization.jsonObject(with: encoder.encode(analysis))
        let legacy: [String: Any] = [
            "id": "legacy-window",
            "interval": [
                "start": snapshot.utcDate.timeIntervalSinceReferenceDate,
                "duration": 86_400.0,
            ],
            "peakDate": snapshot.utcDate.timeIntervalSinceReferenceDate + 43_200,
            "score": 64.0,
            "snapshot": snapshotObject,
            "analysis": analysisObject,
        ]
        let data = try JSONSerialization.data(withJSONObject: legacy)
        let candidate = try JSONDecoder().decode(ElectionTimingCandidate.self, from: data)

        #expect(candidate.assessment.suitabilityScore == 64)
        #expect(candidate.legacyHoraryAnalysis != nil)

        let reencoded = try encoder.encode(candidate)
        let text = String(decoding: reencoded, as: UTF8.self)
        #expect(!text.contains("\"analysis\""))
        #expect(!text.contains("\"score\""))
        #expect(text.contains("\"assessment\""))
    }
    ...
    @Test("Horary deterministic result types round-trip through JSON")
    func horaryResultCodableRoundTrip() async throws {
        let calculator = try SwissEphemerisCalculator(ephemerisDirectory: ephemerisDirectory)
        let snapshot = try await calculator.calculateSnapshot(
            NatalInput(
                utcDate: Date(timeIntervalSince1970: 1_775_000_000),
        ...
        let analysis = HoraryEngine.analyze(snapshot: snapshot, targetHouse: 10)
        let choice = try #require(HoraryEngine.analyzeChoices(
            snapshot: snapshot,
            candidates: [HoraryChoiceCandidate(label: "A", house: 10)]
        ).first)
        let electionalAssessment = ElectionalAssessment(
            suitabilityScore: 72,
            factors: [
                ElectionalFactor(id: "target-condition", contribution: 22),
                ElectionalFactor(id: "moon-condition", contribution: 18),
            ]
        )
        let timing = ElectionTimingCandidate(
            id: "sample-window",
            interval: DateInterval(start: snapshot.utcDate, duration: 86_400),
            peakDate: snapshot.utcDate.addingTimeInterval(43_200),
            snapshot: snapshot,
            assessment: electionalAssessment
        )
        let significator = try #require(HoraryEngine.assessSignificators(
            [HorarySignificator(label: "work", house: 10)],
            snapshot: snapshot
        ).first)
        ...
        let decodedAnalysis = try decoder.decode(HoraryAnalysis.self, from: encoder.encode(analysis))
        let decodedChoice = try decoder.decode(HoraryChoiceResult.self, from: encoder.encode(choice))
        let decodedTiming = try decoder.decode(ElectionTimingCandidate.self, from: encoder.encode(timing))
        let decodedSignificator = try decoder.decode(HorarySignificatorAssessment.self, from: encoder.encode(significator))

        #expect(decodedAnalysis == analysis)
        #expect(decodedChoice == choice)
        #expect(decodedTiming == timing)
        #expect(decodedSignificator == significator)
    }
