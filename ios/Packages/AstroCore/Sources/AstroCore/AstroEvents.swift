...
extension SwissEphemerisCalculator {
    ...
    /// The most recent sign ingress strictly before `date`.
    /// Exposed for traditional techniques that need the current sign's full event sequence.
    public func previousSignIngress(
        for body: CelestialBody,
        before date: Date
    ) throws -> Date {
        try nextSignIngressBackward(for: body, before: date)
    }
