...
func (s *Store) migrate() error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS commerce_users (
			...
			root_user_id TEXT NOT NULL DEFAULT '',
			predecessor_user_id TEXT,
		...
		`CREATE TABLE IF NOT EXISTS apple_credit_claims (
			identity_hash TEXT NOT NULL,
			source TEXT NOT NULL,
			period_key TEXT NOT NULL,
			claimed_at TEXT NOT NULL,
			PRIMARY KEY(identity_hash, source, period_key)
		)`,
		...
		`CREATE TABLE IF NOT EXISTS store_transaction_reconciliations (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			current_user_id TEXT NOT NULL,
			transaction_id TEXT NOT NULL DEFAULT '',
			product_id TEXT NOT NULL DEFAULT '',
			presented_user_id TEXT NOT NULL DEFAULT '',
		...
		`CREATE INDEX IF NOT EXISTS idx_store_transactions_original ON store_transactions(original_transaction_id)`,
		`CREATE INDEX IF NOT EXISTS idx_store_reconciliations_user_created ON store_transaction_reconciliations(current_user_id,created_at DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_app_store_notifications_received ON app_store_notifications(received_at DESC)`,
	...
	for _, column := range []struct{ name, definition string }{
		...
		if err := ensureSQLiteColumn(s.db, "commerce_users", column.name, column.definition); err != nil {
			return fmt.Errorf("migrate commerce user %s: %w", column.name, err)
		}
	}
	if _, err := s.db.Exec(`UPDATE commerce_users SET root_user_id=user_id WHERE root_user_id=''`); err != nil {
		return fmt.Errorf("backfill commerce root user: %w", err)
	}
	if err := ensureSQLiteColumn(s.db, "subscriptions", "environment", "TEXT NOT NULL DEFAULT 'Production'"); err != nil {
		return fmt.Errorf("migrate subscription environment: %w", err)
	}
	if err := ensureSQLiteColumn(s.db, "store_transactions", "environment", "TEXT NOT NULL DEFAULT 'Production'"); err != nil {
		return fmt.Errorf("migrate transaction environment: %w", err)
	}
...
func (s *Store) ListPrompts() ([]map[string]any, error) {
	rows, err := s.db.Query(`SELECT scope, locale, system_prompt, version, updated_at FROM prompt_templates WHERE locale = ? ORDER BY scope`, canonicalPromptLocale)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
