...
func TestUnacknowledgedReportReleasesItsOriginalCredit(t *testing.T) {
	...
	if balance.Total != monthlyBonus || balance.Allowance != monthlyBonus || balance.Bonus != 0 || balance.Reserved != 0 {
...
func TestStoreTransactionReplayAndAnnualWelcomeAreIdempotent(t *testing.T) {
	...
	if balance.Purchased != 10 || balance.Bonus != 20 || balance.Allowance != monthlyBonus+premiumAllowance || balance.Total != 10+monthlyBonus+20+premiumAllowance {
...
func TestVerifiedSubscriptionGracePeriodKeepsPremiumActive(t *testing.T) {
	...
	if user.Credits.Bonus != 0 || user.Credits.Allowance != monthlyBonus+premiumAllowance {
...
func TestTwentyCreditPurchaseAndAdminResetPreserveNonAdminCredits(t *testing.T) {
	...
	if user.Credits.Purchased != 20 || user.Credits.Bonus != 0 || user.Credits.Allowance != monthlyBonus || user.Credits.Total != 20+monthlyBonus {
...
func TestAdminPlanOverrideSwitchesFreePremiumAndAuto(t *testing.T) {
	...
	if err != nil || premium.Plan != "premium" || premium.AdminPlanOverride != "premium" || premium.Credits.Allowance != premiumAllowance+monthlyBonus || premium.Credits.Bonus != 0 || premium.Credits.Total != premiumAllowance+monthlyBonus {
	...
	if err != nil || free.Plan != "free" || free.AdminPlanOverride != "free" || free.Credits.Allowance != monthlyBonus || free.Credits.Bonus != 0 || free.Credits.Total != monthlyBonus {
